...
@Composable
fun LocationsListRoute(onLocationClick: (Int) -> Unit, vm: LocationsListViewModel = viewModel()) {
    ...
    when {
        ...
        else         -> LocationsListScreen(onLocationClick = onLocationClick)
    }
}

@Composable
fun LocationDetailRoute(onBack: () -> Unit, vm: LocationDetailViewModel = viewModel()) {
    val ui = vm.ui.collectAsStateWithLifecycle().value
    when {
        ui.isLoading -> LoadingLayout(onClickAnywhere = { vm.forceErrorFromLoading() })
