...
@Composable
fun CharactersListRoute(onCharacterClick: (Int) -> Unit, vm: CharactersListViewModel = viewModel()) {
    ...
    when {
        ...
        else         -> CharactersScreen(onCharacterClick = onCharacterClick)
    }
}


@Composable
fun CharacterDetailRoute(onBack: () -> Unit, vm: CharacterDetailViewModel = viewModel()) {
    val ui = vm.ui.collectAsStateWithLifecycle().value
    when {
        ui.isLoading -> LoadingLayout(onClickAnywhere = { vm.forceErrorFromLoading() })
